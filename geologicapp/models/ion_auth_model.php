...
class Ion_auth_model extends CI_Model
{
	...
	/**
	 * Salt generation code https://github.com/ircmaxell/password_compat/blob/master/lib/password.php
	 **/
	...
	public function create_center($center_name = FALSE, $center_description = '', $additional_data = array())
	{
		if(!$center_name)
		{
			$this->set_error('center_name_required');
			return FALSE;
		}

		
		$existing_center = $this->db->get_where($this->tables['centros'], array('name' => $center_name))->num_rows();
		...
		$data = array('name'=>$center_name,'description'=>$center_description);

		if (!empty($additional_data)) $data = array_merge($this->_filter_data($this->tables['centros'], $additional_data), $data);

		$this->trigger_events('extra_center_set');

		$this->db->insert($this->tables['centros'], $data);
		$center_id = $this->db->insert_id();

		$this->set_message('center_creation_successful');
